...
const selectedCountry = ({ data, arrayBorderNames }) => {
    return (
    <>
        {data.status == '404' ? <Layout title={'Error | Countries API'}>
            <div className="my-14">
            <img className="mx-auto" src="https://http.cat/404" alt="code_cat" />
            </div>
            
        </Layout> : <Layout title={ '| Countries API'}>
            <Link href="/">
                <button className="px-4 py-1 my-14 bg-dark_blue rounded-sm text-custom_white"> ← Back </button>
            </Link>
                ...
                <div className="py-8 text-custom_white">
                    ...
                    <div className="flex gap-4 my-8 flex-col md:flex-row w-full">
                        <div className="md:w-3/6 w-full">
                            <h3 className="my-2"> <span className="font-semibold"> Official name : </span> {data[0].name.official}  </h3>
                            <h3 className="my-2"> <span className="font-semibold"> Population : </span> {data[0].population.toLocaleString("en-US")}  </h3>
                            <h3 className="my-2"> <span className="font-semibold"> Region : </span> {data[0].region? data[0].region : '---'}  </h3>
                            <h3 className="my-2"> <span className="font-semibold"> Subregion : </span> {data[0].subregion ? data[0].subregion : ' --- '}  </h3>
                            <h3 className="my-2"> <span className="font-semibold"> Capital : </span> {
                        ...
                        <div className="md:w-3/6 w-full">
                            ...
                            <h3 className="my-2"> <span className="font-semibold"> Languages : </span> {
                                data[0].languages ? Object.values(data[0].languages).toString() : ' --- '
                            }  </h3>
                        </div>
                    </div>
...
export async function getStaticProps({ params }) {
    try {
       ...
       // console.log(data[0].borders)
        ...
        //const result = dataGlobal.find(element => element.cca3 == data[0].cca3)
        
       // console.log(result.borders)
/*
